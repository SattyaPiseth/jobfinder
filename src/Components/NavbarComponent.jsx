...
const MenuList = ({ isLoading, menuList, fontClass }) => (
  <Navbar.Collapse className={`md:flex justify-center space-x-4 ${isLoading ? "block" : "hidden"}`}>
    {isLoading
      ? Array(5).fill(0).map((_, index) => (
          <div key={index} className="mx-2">
            <Skeleton height={30} width={100} className="rounded animate-pulse" />
          </div>
        ))
    ...
    <Navbar fluid className="bg-primary-800 shadow-md fixed top-0 left-0 right-0 z-50">
      <div className="container mx-auto flex flex-wrap items-center justify-between py-2 px-4">
        <Navbar.Brand href="/" className="flex items-center">
          {isLoading ? (
            <Skeleton circle height={36} width={36} className="mr-3 animate-pulse" />
          ) : (
            <img
              alt="Logo"
              loading="lazy"
        ...
        <div className="flex flex-grow justify-center order-2 md:order-1 w-full md:w-auto">
          <MenuList isLoading={isLoading} menuList={menuList} fontClass={fontClass} />
        </div>

        <div className="relative flex order-1 md:order-2 items-center gap-x-6">
          <LanguageDropdown fontClass={fontClass} />
          {isLoading ? (
            <Skeleton circle height={40} width={40} className="animate-pulse" />
          ) : isAuthenticated ? (
            <Dropdown
              arrowIcon={false}
              inline
